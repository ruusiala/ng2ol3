...
First run ```npm install``` and ```npm run typings install``` to download all dependencies (~180MB). It is highly recommended to install typescript and node-sass system-wide: ```npm install -g typescript``` and ```npm install -g node-sass```.
